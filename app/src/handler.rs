...
pub async fn send_sms(body: web::Bytes, session: Session) -> Result<String, HttpResponse> {
    ...
    let phone_number: PhoneReq =
        serde_json::from_str(&phone_number_str).map_err(utils::internal_server_error)?;
    ...
    conn.execute(
        "INSERT INTO users (phone, token)
                  VALUES ($1, $2)",
        &[phone_number_str.clone(), token],
    )
    .unwrap();
    ...
    conn.execute(
        "INSERT INTO sms_codes (id, secret_code)
                VALUES ($1, $2)",
        &[id.clone().to_string(), secret.clone()],
    )
    .unwrap();
...
pub async fn verify_code(body: web::Bytes, session: Session) -> Result<String, HttpResponse> {
    ...
    let CorrectCode { correct_code } = stmt
...
pub async fn auth(body: web::Bytes, session: Session) -> Result<String, HttpResponse> {
    ...
    let IdResp { id } = stmt
        .query_row(rusqlite::params![token.token], |row| {
            Ok(IdResp {
                id: row.get(0).unwrap(),
            })
        })
...
pub async fn ready(
    body: web::Bytes,
    session: Session,
    actix_data: web::Data<Arc<Mutex<Keys>>>,
    ...
    let conn = utils::db_connection();
    conn.execute(
        "INSERT INTO sign_process (id, blinded_digest, subset, judge_pubkey)
                  VALUES ($1, $2, $3, $4)",
        &[
            id.to_string(),
            blinded_digest_str,
...
pub async fn sign(
    ...
    let conn = utils::db_connection();
    let mut stmt = conn
        .prepare("SELECT blinded_digest, subset, judge_pubkey FROM sign_process WHERE id=?")
        .expect("failed to select");
    ...
    conn.execute("DELETE FROM sign_process WHERE id=?", &[id])
        .unwrap();
